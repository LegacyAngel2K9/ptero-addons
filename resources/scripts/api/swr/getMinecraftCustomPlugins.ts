...
export default (sectionId: string, uuid: string) => {
    ...
    return useSWR<PaginatedResult<any>>([ 'server:minecraftPlugins', page, searchFilter ], async () => {
        const { data } = await http.get('/api/client/getcustomplugins', { params: { size: 20, page: page, sectionId, searchFilter, uuid }, timeout: 60000 });

        return ({
            items: (data || []),
            pagination: { total: 13170, count: data.length, perPage: 20, currentPage: page, totalPages: data.length / 20 },
        });
    });
};
