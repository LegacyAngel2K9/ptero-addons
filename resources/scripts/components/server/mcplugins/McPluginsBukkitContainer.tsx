...
import getMinecraftBukkitPlugins, { Context as ServerPluginsContext } from '@/api/swr/getMinecraftBukkitPlugins';
import McPluginsBukkitRow from './McPluginsBukkitRow';
import Spinner from '@/components/elements/Spinner';
interface Values {
    search: string;
}

const McPluginsBungeeContainer = () => {
    const { clearFlashes, clearAndAddHttpError } = useFlash();
    const uuid = ServerContext.useStoreState(state => state.server.data!.uuid);
    const serverId = ServerContext.useStoreState(state => state.server.data!.id);
    const { page, setPage, searchFilter, setSearchFilter } = useContext(ServerPluginsContext);

    const { data: minecraftPlugins, error, isValidating } = getMinecraftBukkitPlugins(uuid);
            ...
            <Pagination data={minecraftPlugins} onPageSelect={setPage}>
                {({ items }) => (
                    !items.length ?
                        ...
                        :
                        items.map((minecraftPlugins, index) => <McPluginsBukkitRow
