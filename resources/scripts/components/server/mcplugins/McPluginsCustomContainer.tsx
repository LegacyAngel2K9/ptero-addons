...
import McPluginsRow from './McPluginsBukkitRow';
import { ServerContext } from '@/state/server';
import { Form, Formik } from 'formik';
import Field from '@/components/elements/Field';
import { object, string } from 'yup';
import Pagination from '@/components/elements/Pagination';
import getMinecraftCustomPlugins, { Context as ServerPluginsContext } from '@/api/swr/getMinecraftCustomPlugins';
interface Values {
    search: string;
}

const McPluginsCustomContainer = () => {
    const { clearFlashes, clearAndAddHttpError } = useFlash();
    const uuid = ServerContext.useStoreState(state => state.server.data!.uuid);
    const serverId = ServerContext.useStoreState(state => state.server.data!.id);
    const { page, setPage, searchFilter, setSearchFilter } = useContext(ServerPluginsContext);

    const { data: minecraftPlugins, error, isValidating } = getMinecraftCustomPlugins('2', uuid);
